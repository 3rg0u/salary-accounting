...
@section('content')
    ...
    <div class="header">
        <p style="text-align: center" class="h1 alert alert-info">Thông Tin Hóa Đơn Tiền Lương Của Giảng Viên</p>
        <p style="text-align: center" class="h1 alert alert-info">Học Kỳ {{$semester->code}}</p>
    </div>
    ...
    <div class="body">
        <div class="navigation d-flex justify-content-start gap-2">
            <button class="btn btn-secondary btn-lg d-flex align-items-center gap-1"
                onclick="window.location.href='{{ route('accountant.salary.index') }}'">
                        ...
                        @foreach ($prof_bills as $prof_id => $bills)
                            <tr>
                                ...
                                <td>
                                    ...
                                    @include('accountant.salary.components.class', ['prof_id' => $prof_id, 'classes' => $bills])
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
