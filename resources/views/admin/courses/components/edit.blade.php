<div class="modal fade" id="_editCourse_{{$course->code}}" tabindex="-1" aria-labelledby="editInfor" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="editInfor">Cập nhật thông tin học phần {{$course->name}}</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form action="{{route('admin.courses.update', ['code' => $course->code])}}" method="POST">
                    @method('PUT')
                    @csrf
                    <div class="mb-3">
                        <label for="name" class="form-label">Tên học phần:</label>
                        <input type="text" class="form-control w-100" id="name" name="name" value="{{$course->name}}">
                    </div>
                    <div class="mb-3">
